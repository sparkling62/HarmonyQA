...
class InternVLChatModel(PreTrainedModel):
    ...
    def __init__(self, config: InternVLChatConfig, vision_model=None, language_model=None, use_flash_attn=True):
        ...
        self.sparsevit_mlp = nn.Sequential(
            nn.Linear(768, 4096),
            nn.GELU(),
            nn.Linear(4096, 4096)
        )
    ...
    def forward(
        ...
        # Predict MOS score from the 4th-to-last token
        last_hidden_states = outputs.hidden_states[-1]
        input_tensor = last_hidden_states[:, -4, :]
        if torch.isnan(input_tensor).any():
            input_tensor = torch.nan_to_num(input_tensor, nan=0.0, posinf=1e9, neginf=-1e9)
        score1 = self.mlpscore(input_tensor).squeeze(1)
        loss1 = nn.L1Loss(reduction='mean')(score1, mos)
