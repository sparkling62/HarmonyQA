        ...
        for item2 in tqdm(test_dataloader, desc="Evaluating"):
            ...
            for k, v in item2.items():
                ...
                if isinstance(v, torch.Tensor):
                    ...
                    # 如果是 tensor 列表，也移动到 device
                    item[k] = [t.to(model.device) for t in v]
                else:
                    item[k] = v  # 保留原样
            ...
            forward_args = {
                "mos": item["mos"],
                "pixel_values": item["pixel_values"].squeeze(0).to(torch.bfloat16),
                "input_ids": item["input_ids"],
                "attention_mask": item["attention_mask"],
                "image_flags": item["image_flags"].squeeze(0),
                "labels": item["labels"]
            }

            # 如果是视频帧序列，添加 pixel_values2（确保是 list of tensors）
            if 'pixel_values2' in item and isinstance(item['pixel_values2'], list) and len(item['pixel_values2']) > 0:
                forward_args["pixel_values2"] = [
                    frame.squeeze(0).to(torch.bfloat16) for frame in item["pixel_values2"]
                ]
