...
from model.internvl_chat_st2 import (InternVisionConfig,
                                          InternVisionModel,
                                          InternVLChatConfig,
                                          InternVLChatModel)
...
class CustomTrainer(Trainer):
    ...
    def evaluate(self, eval_dataset=None,ignore_keys=None):
        ...
        with torch.no_grad():
            for item2 in eval_dataloader:
                ...
                for k, v in item2.items():
                    if k in ['video_name', 'answer']:
                        continue
                    if isinstance(v, torch.Tensor):
                        item[k] = v.to(self.model.device)
                    else:
                        item[k] = v
                ...
                forward_args = {
                    "mos": item["mos"],
                    "pixel_values": item["pixel_values"].squeeze(0).to(torch.bfloat16).to(self.model.device),
                    "input_ids": item["input_ids"].to(self.model.device),
                    "attention_mask": item["attention_mask"].to(self.model.device),
                    "image_flags": item["image_flags"].squeeze(0).to(self.model.device),
                    "labels": item["labels"].to(self.model.device)
                }

                # 如果是视频，才添加 pixel_values2
                if 'pixel_values2' in item and isinstance(item['pixel_values2'], list) and len(
                        item['pixel_values2']) > 0:
                    forward_args["pixel_values2"] = [
                        frame.squeeze(0).to(torch.bfloat16).to(self.model.device)
                        for frame in item["pixel_values2"]
                    ]
        ...
        # 计算 SRCC, PLCC, KRCC 三个指标的平均值
        current_avg = (metrics["SRCC_score"] + metrics["PLCC_score"] + metrics["KRCC_score"]) / 3

        # Track best average metric
        if current_avg > self.best_metric:
            self.best_metric = current_avg
            print(
                f"New best average metric: {self.best_metric:.4f} (SRCC: {metrics['SRCC_score']:.4f}, PLCC: {metrics['PLCC_score']:.4f}, KRCC: {metrics['KRCC_score']:.4f}). Saving model...")
...
class LazySupervisedDataset(Dataset):
    ...
    def multi_modal_multi_image_get_item(self, data_item):
        ...
        # ----------- 自动插入 <image> 占位符 -----------
        conv_text = conversations[0]['value']
        num_placeholders = conv_text.count('<image>')
        if num_placeholders < num_image:
            # 不足时追加占位符（通常直接按顺序接在末尾）
            extra = num_image - num_placeholders
            if not conv_text.endswith('\n'):
                conv_text += '\n'
            conv_text += '\n'.join([f'Image{i + 1}: <image>' for i in range(num_placeholders, num_image)])
            conversations[0]['value'] = conv_text
        assert conversations[0]['value'].count('<image>') == num_image, "conversations中<image>数量应与image数量一致"
        # ---------------------------------------------
    ...
    def video_get_item(self, data_item):
        ...
        video_list = data_item['video']  # list of video filenames
        assert isinstance(video_list, list), "data_item['video'] 应该是一个视频路径的列表"

        all_images = []
        all_sampled = []
        conv_text = data_item['conversations'][0]['value']
        video_count = conv_text.count('<video>')
        assert video_count == len(video_list), "conversations 中 <video> 的数量应与 video 列表长度一致"
        ...
        for vid_idx, video_file in enumerate(video_list):
            ...
            all_sampled.append(sampled_video)

            special_tokens = '\n'.join([f'Video{vid_idx + 1}-Frame{i + 1}: <image>' for i in range(len(image_list))])
            conv_text = conv_text.replace('<video>', special_tokens, 1)

        # 添加 motion tokens: 每个视频4个
        motion_tokens = '\n'.join([
            f'Motion{vid_idx + 1}: <image><image><image><image>'
            for vid_idx in range(len(video_list))
        ])
        conv_text = motion_tokens + '\n' + conv_text
        data_item['conversations'][0]['value'] = conv_text

        # 图像预处理
        pixel_values = [transform(image) for image in all_images]
        pixel_values = torch.stack(pixel_values)

        # 图像 token 统计
        num_motion_tokens = 4 * len(video_list)
        num_patches = len(all_images) + num_motion_tokens
        num_image_tokens = [self.num_image_token] * num_patches
        for i in range(num_motion_tokens):
            num_image_tokens[-(i + 1)] = 1
        ...
        ret = dict(
            ...
            attention_mask=ret['attention_mask'][0],
            pixel_values=pixel_values,
            pixel_values2=all_sampled,  # List of sampled videos
            image_flags=torch.tensor([1] * (num_patches - num_motion_tokens), dtype=torch.long)
        )
        return ret
